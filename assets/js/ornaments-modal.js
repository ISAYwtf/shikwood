let close = document.querySelector(".close"),
    ...
    btnDxf = document.querySelector(".btn-dxf"),
    btnStl = document.querySelector(".btn-stl"),
    radioSize = document.forms.sizeInfo;
...
btnStl.addEventListener("click", (event) => {
    let target = event.target,
        link = document.createElement('a'),
        title = modalTitle.textContent.split(" "),
        size = sizeInfo.elements.size.value;

    event.preventDefault();

    if (size != "") {
        link.setAttribute('href', `assets/files/${title[1]}-${size}.STL`);
        link.click();
    }
});

btnDxf.addEventListener("click", (event) => {
    let target = event.target,
        link = document.createElement('a'),
        title = modalTitle.textContent.split(" "),
        size = sizeInfo.elements.size.value;
    ...
    if (size != "") {
        link.setAttribute('href', `assets/files/${title[1]}-${size}.dxf`);
        link.click();
    }
});